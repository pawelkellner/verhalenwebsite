import { StoryCardProps } from "./components/story-card/story-card.types";

export const stories: StoryCardProps[] = [
    {
        id: 101,
        title: "Peter en de Powerpoint",
        image: "https://picsum.photos/201",
        text: "Peter was een gewone kantoormedewerker tot hij op een dag een magische presentatie ontdekte. Met behulp van zijn Powerpoint-vaardigheden werd hij een held in zijn kantoor.",
        author: "Marjolein de Vries",
        songName: "Queen - Bohemian Rhapsody"
    },
    {
        id: 102,
        title: "Het Geheim van de Verborgen Schat",
        image: "https://picsum.photos/212",
        text: "Een groep avonturiers gaat op zoek naar een lang verloren schat diep begraven onder de aarde. Maar ze zijn niet de enigen die de schat willen vinden, en ze moeten strijden tegen boosaardige krachten om hun doel te bereiken.",
        author: "Jan van Dijk",
        songName: "Led Zeppelin - Stairway to Heaven"
    },
    {
        id: 103,
        title: "Een Reis Door de Tijd",
        image: "https://picsum.photos/220",
        text: "In een experiment met een tijdmachine worden drie vrienden per ongeluk teruggezonden naar het oude Egypte. Ze moeten een manier vinden om terug naar huis te komen, terwijl ze proberen de tijdlijn niet te verstoren.",
        author: "Sophie de Boer",
        songName: "Michael Jackson - Billie Jean"
    },
    {
        id: 104,
        title: "Het Betoverde Bos",
        text: "Een jong meisje genaamd Anna ontdekt een betoverd bos achter haar huis. Ze ontmoet magische wezens en leert over vriendschap en moed terwijl ze probeert de vloek die op het bos rust te verbreken.",
        author: "Thomas Jansen",
        songName: "The Beatles - Hey Jude"
    },
    {
        id: 105,
        title: "Ruimte Odyssee",
        image: "https://picsum.photos/230",
        text: "Een groep astronauten vertrekt op een spannende missie naar de verre uithoeken van het heelal. Tijdens hun reis worden ze geconfronteerd met buitenaardse wezens, ruimtepiraten en de mysteries van het universum.",
        author: "Lisa van der Meer",
        songName: "David Bowie - Space Oddity"
    }
];
